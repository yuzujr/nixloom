...
function useCompactChatTimestamp(): boolean {
  return (
...
function formatCompactChatTimestamp(date: Date, now: Date): string {
...
export function formatChatTimestampForDisplay(
  timestamp: number,
  ...
  return {
    label:
      (options.compact ?? useCompactChatTimestamp())
        ? formatCompactChatTimestamp(date, new Date())
        : date.toLocaleString([], {
            ...hourCycle,
            month: "short",
            day: "numeric",
