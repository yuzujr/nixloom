...
describe("grouped chat rendering", () => {
  ...
  it("uses a compact timestamp on the phone", () => {
    const today = new Date();
    today.setHours(9, 5, 0, 0);
    expect(formatChatTimestampForDisplay(today.getTime(), { compact: true }).label).toBe("09:05");

    const earlier = new Date(today);
    earlier.setDate(earlier.getDate() - 1);
    earlier.setHours(18, 30, 0, 0);
    expect(formatChatTimestampForDisplay(earlier.getTime(), { compact: true }).label).toBe(
      `${earlier.getMonth() + 1}/${earlier.getDate()} 18:30`,
    );
  });
