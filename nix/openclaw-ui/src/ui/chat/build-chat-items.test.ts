...
function groupAt(groups: MessageGroup[], index: number): MessageGroup {
...
describe("buildChatItems", () => {
  it("coalesces adjacent tool calls and results into one activity item", () => {
    ...
    expect(group.role).toBe("tool");
    expect(group.messages).toHaveLength(1);
    const cards = extractToolCards(group.messages[0]?.message, "coalesced");
    ...
    expect(firstMessageContent(group)).toContainEqual({
      type: "image",
      data: "fixture-image",
      mimeType: "image/png",
    });
  ...
  it("coalesces interleaved parallel call/result pairs by call id", () => {
    ...
    const cards = group.messages.flatMap((entry, index) =>
  ...
  it("replaces a repeated unresolved single-call snapshot", () => {
    ...
    expect(groups).toHaveLength(1);
    expect(group.messages).toHaveLength(1);
    const cards = extractToolCards(group.messages[0]?.message, "repeated-snapshot");
    expect(cards).toHaveLength(1);
    expect(cards[0]).toMatchObject({
      callId: "call-x",
      args: { path: "new.ts" },
  ...
  it("keeps more than sixteen parallel calls open by call id", () => {
    ...
    const group = groupAt(groups, 0);
    expect(groups).toHaveLength(1);
    expect(group.messages).toHaveLength(17);
    const cards = group.messages.flatMap((entry, index) =>
  ...
  it("coalesces a canonical multi-call assistant message with standalone results", () => {
    ...
    expect(group.role).toBe("tool");
    expect(group.messages).toHaveLength(1);
    const cards = extractToolCards(group.messages[0]?.message, "canonical-parallel");
    expect(cards).toHaveLength(2);
    expect(cards[0]).toMatchObject({
      callId: "call-a",
      args: { path: "a.ts" },
  ...
  it("coalesces provider-shaped result blocks by canonical tool-use id", () => {
    ...
    expect(groups).toHaveLength(1);
    expect(group.messages).toHaveLength(1);
    const cards = extractToolCards(group.messages[0]?.message, "provider-coalesced");
    expect(cards).toHaveLength(1);
    expect(cards[0]).toMatchObject({
