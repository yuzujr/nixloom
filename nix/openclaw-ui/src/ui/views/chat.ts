...
function openComposerFilePicker(target: HTMLElement): void {
  target
...
function clickComposerFileInput(event: MouseEvent): void {
  if (!(event.currentTarget instanceof HTMLElement)) {
    return;
  }
  openComposerFilePicker(event.currentTarget);
}

function setComposerActionsOpen(
  open: boolean,
  requestUpdate: () => void,
...
function trapComposerActionsFocus(event: KeyboardEvent): void {
  if (event.key !== "Tab") {
    return;
  }
  ...
  const actions = Array.from(sheet.querySelectorAll<HTMLButtonElement>("button:not(:disabled)"));
  if (actions.length === 0) {
    return;
  }
...
function renderComposerActions(
  ...
  if (!vs.composerActionsOpen) {
    return nothing;
  }

  const close = (restoreFocus = false) =>
    setComposerActionsOpen(false, requestUpdate, { restoreFocus });
  ...
  return html`
    <button
      class="nixloom-composer-actions__backdrop"
      type="button"
      aria-label="Close message actions"
      @click=${() => close(true)}
    ></button>
    <section
      id="nixloom-composer-actions"
      class="nixloom-composer-actions"
      role="dialog"
      aria-modal="true"
      ...
      @keydown=${(event: KeyboardEvent) => {
        ...
        trapComposerActionsFocus(event);
      }}
    >
      <button
        ...
        @click=${(event: MouseEvent) => {
          if (event.currentTarget instanceof HTMLElement) {
            run(() => openComposerFilePicker(event.currentTarget as HTMLElement));
          }
        }}
      >
      ...
      <button
        ...
        type="button"
        @click=${() => {
          vs.composerContextOpen = !vs.composerContextOpen;
          close();
        }}
      >
...
export function renderChat(props: ChatProps) {
  ...
  const chatColumnFooter = html`
    ...
    <!-- Input bar -->
    <div
      class="agent-chat__input ${vs.composerContextOpen ? "agent-chat__input--context-open" : ""}"
    ...
    >
      ...
      ${renderComposerActions(props, requestUpdate)}
